...
<body>
    ...
    def _generate_html_for_s3(self, markdown_path, custom_domain=None):
        """
        Generate HTML for S3 with proper image paths (not CID references)

        Args:
            markdown_path: Path to Markdown report file
            custom_domain: CloudFront custom domain

        Returns:
            HTML string with embedded CSS and proper image paths
        """
        # Read Markdown content
        with open(markdown_path, 'r', encoding='utf-8') as f:
            markdown_content = f.read()
...
<body>
    <div class="container">
        ...
        # Keep original image paths for S3 (they will be uploaded alongside the HTML)
        # No need to replace with CID references for web viewing
...
{reports_html}
    ...
    def upload_to_s3(self, html_content, markdown_path):
        """
        Upload HTML content and images to S3 bucket and update index page

        Args:
            html_content: HTML content (ignored, will generate S3-specific HTML)
            markdown_path: Path to markdown file (for extracting date and images)
        ...
        try:
            ...
            print(f"Uploading to S3: s3://{bucket_name}/{html_key}")
            ...
            self._upload_images_to_s3(s3_client, bucket_name, markdown_path, images_prefix)

            # Generate S3-specific HTML (without CID references)
            print("Generating S3-optimized HTML...")
            s3_html_content = self._generate_html_for_s3(markdown_path, custom_domain)

            # Upload HTML content
            s3_client.put_object(
                Bucket=bucket_name,
                Key=html_key,
                Body=s3_html_content.encode('utf-8'),
                ContentType='text/html',
                ContentEncoding='utf-8'
            )

            # Generate S3 URL
            s3_url = f"https://{bucket_name}.s3.{region}.amazonaws.com/{html_key}"
            if custom_domain:
                s3_url = f"https://{custom_domain}/{html_key}"
    ...
    def _upload_images_to_s3(self, s3_client, bucket_name, markdown_path, images_prefix):
        """
        Upload images referenced in markdown to S3

        Args:
            s3_client: boto3 S3 client
            bucket_name: S3 bucket name
            markdown_path: Path to markdown file
            images_prefix: S3 key prefix for images
        """
        markdown_dir = Path(markdown_path).parent
        ...
        for img_path in image_paths:
            ...
            # Build S3 key for image
            # Keep the relative path structure (e.g., images/chart.png)
            img_s3_key = f"{images_prefix}/{Path(img_path).name}"
            ...
            try:
                ...
                uploaded_count += 1
                print(f"  ✅ Uploaded: {img_path} → s3://{bucket_name}/{img_s3_key}")

            except Exception as e:
                print(f"  ⚠️  Failed to upload {img_path}: {e}")

        print(f"✅ Uploaded {uploaded_count} image(s) to S3")
