...
description: "Retrieve and query GenAI project raw data from DynamoDB tables (github-insight-raw-data, github-trend-repo-candidates, and genai-repo-watchlist). With the data, 1) you can get the analysis of repository on the specified date. 2) you can get the repository in the github trending page on the specified date. 3) you can get the priority/tier classification of repositories."
---
...
Retrieve raw data from DynamoDB tables for GenAI project insights. Returns JSON-formatted data from:
- `github-insight-raw-data` (repo analysis)
- `github-trend-repo-candidates` (trending repos)
- `genai-repo-watchlist` (repo priority classification)
...
**For github-insight-raw-data and github-trend-repo-candidates:**
```bash
python scripts/dynamodb_manager.py [--table TABLE] get <project_url> <collect_date> [--json]
```

**For genai-repo-watchlist (no date required):**
```bash
python scripts/dynamodb_manager.py --table genai-repo-watchlist get <project_url> [--json]
```

### 2. Query by Date Range
Query historical data for a repository within a date range.

**Note:** Only available for github-insight-raw-data and github-trend-repo-candidates tables.
```bash
...
**For genai-repo-watchlist (no date required, overwrites existing record):**
```bash
python scripts/dynamodb_manager.py --table genai-repo-watchlist put <project_url> --data '{"priority": "high"}'
```
...
**For genai-repo-watchlist (no date required):**
```bash
python scripts/dynamodb_manager.py --table genai-repo-watchlist delete <project_url> [--confirm]
```

## Data Schema

### github-insight-raw-data and github-trend-repo-candidates
...
- **Partition Key**: `project_url` (String) - GitHub repository URL
- **No Sort Key**: Each repository has only one record that gets overwritten on update
- **Attributes**: `priority` (String) - Priority/tier level; `last_updated` (String, Optional) - Last update timestamp
...
- **genai-repo-watchlist**: Contains current priority/tier classification for repositories. Each repository has only one record that gets updated (overwritten) when classification changes
...
### Example 5: Get Repository Priority Classification
...
**Use case**: Retrieve the current priority/tier classification for a specific repository.

### Example 6: Add or Update Priority Classification
...
**Use case**: Store or update the classification result for a trending repository. This will overwrite any existing classification.
...
### 3. Repository Priority Management
Use `genai-repo-watchlist` to store and query priority classifications for trending repositories (e.g., high, medium, low priority).

### 4. Cross-Repository Comparison
Use scan or multiple queries to compare different repositories on the same date.

### 5. Time-Series Data Export
...
### Global Parameters
- `--table`: Choose table (`github-insight-raw-data`, `github-trend-repo-candidates`, or `genai-repo-watchlist`). Default: `github-insight-raw-data`
